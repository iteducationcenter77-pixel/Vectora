...
// Detect serverless environment (Vercel / AWS Lambda / Azure Functions)
const isServerless = !!(process.env.VERCEL || process.env.AWS_REGION || process.env.FUNCTIONS_WORKER_RUNTIME);
if (isServerless) {
  console.log('Detected serverless environment — skipping local-only server setup in server.js');
  // Export app for completeness, but do not perform filesystem operations or listen.
  module.exports = app;
} else {
  app.use(cors());
